...
`$ run_beta_mining.py -f <DIRECTORY>` or `$ run_beta_mining.py --filepath <DIRECTORY>` will perform the `beta_mining` algorithm on the directory indicated, with all options set to default values. A config YAML file will be generated in the directory to serve as a record of options that were used during the script execution.
